...
function Example:LNR_ON_NEW_PLATE(eventname, plateFrame, plateData)
    ...
    print(ADDON_NAME, ":", 'direct region access test: (level)', self:GetPlateRegion(plateFrame, 'level'):GetText(), 'has raid icon?', self:GetPlateRegion(plateFrame, 'raidIcon'):IsShown() and 'YES!' or 'no');
end
